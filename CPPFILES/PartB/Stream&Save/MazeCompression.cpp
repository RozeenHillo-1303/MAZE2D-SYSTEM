...
MazeCompression::MazeCompression(std::vector<MultiTypes*> data, std::string path) {
	if (path == "") throw "have not path";
	m_path = path;
...
MazeCompression::MazeCompression(const MazeCompression& comp) {
	if (comp.m_path == "") throw "have not path";
	if (comp.m_data.empty()) return;
	for (auto& d : comp.m_data)
		m_data.push_back(d);
	m_path = comp.m_path;
}

void MazeCompression::compress() { // לדחוס
	if (m_data.empty()) throw "have not data";
	...
	char prev = -1;
	int counter = 1;
	for (int i = 7; i < (int)m_data.size(); i++) {
...
std::vector<MultiTypes*> MazeCompression::dicompress() { // יציאה מהדחיסה
	std::ifstream infile; infile.open(m_path, std::ios::in | std::ios::binary);
	if (!infile.is_open())throw "Error: cannot open file or the path does't correct";
	int rows, cols;
